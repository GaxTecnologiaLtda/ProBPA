...
async function run() {
    ...
    const phantomCnesSet = new Set<string>();
    ...
    snap.forEach(d => {
        ...
        if (recCnes && !cnesMap.has(recCnes)) {
            phantomCnesSet.add(recCnes);
        }
    });

    console.log(`\nCNES Fantasmas Encontrados (Enviados pelo Conector mas não cadastrados no Painel):`);
    for (const cnes of phantomCnesSet) {
        console.log(`- ${cnes}`);
    }
}
