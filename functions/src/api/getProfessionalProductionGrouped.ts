...
export const getProfessionalProductionGrouped = functions
    ...
    .https.onCall(async (data, context) => {
        ...
        const manualPromises = competencesToSearchManual.map(async (compM) => {
            ...
            manualSnap.docs.forEach(doc => {
                try {
                    ...
                    rawRecords.push({ ...row, source: 'manual' });
                } catch (errInner) {
            ...
            return competencesToSearchConnector.map(async (comp) => {
                try {
                    ...
                    recSnap.docs.forEach(d => {
                        ...
                        rawRecords.push({ ...row, source: 'connector', professionalId: finalId });
                        } catch (errInner) {
        ...
        // 3. Normalize and pre-aggregate by Date and Procedure
        // Using a map to combine identical Date+Procedure within this API payload to reduce bytes sent exactly as frontend handles it
        ...
        rawRecords.forEach(r => {
            ...
            const procCode = code || r.procedureCode || '-';
            const procName = name || r.procedureName || 'Não Identificado';
            const qty = Number(r.quantity) || 1;
            const isCanceled = r.status === 'canceled';

            const aggKey = `${attDate}|${procCode}|${procName}`;
            ...
            if (!aggregatedMap.has(aggKey)) {
                aggregatedMap.set(aggKey, {
                    ...
                    quantity: isCanceled ? 0 : qty,
                    canceledQuantity: isCanceled ? qty : 0,
                    professionalName: r.professional?.name || r.professionalName || professionalName || 'Não Identificado',
                    professionalCns: r.professional?.cns || r.professionalCns || professionalCns || '',
                    cbo: r.professional?.cbo || r.cbo || ''
                });
            } else {
                const existing = aggregatedMap.get(aggKey);
                if (isCanceled) {
                    existing.canceledQuantity = (existing.canceledQuantity || 0) + qty;
                } else {
                    existing.quantity += qty;
                }
                aggregatedMap.set(aggKey, existing);
            }
        });
