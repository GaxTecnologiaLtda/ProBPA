...
import { Search, Plus, MapPin, AlertCircle, CheckCircle, FileText, UserCheck, Stethoscope, Hammer, ClipboardList, Syringe, User, Layout, Activity, ChevronUp, ChevronDown, Calendar } from 'lucide-react';
...
export const Register: React.FC = () => {
    ...
    const isSimplifiedMode = useMemo(() => {
        ...
        // Cast to any to bypass potential strict type lagging if not picked up, though type is updated.
        const unitAny = currentUnit as any;
        const iface = unitAny.municipalityInterfaceType;

        console.log('[Register] Checking Simplified Mode:', {
            unitId: currentUnit.id,
            municipalityId: currentUnit.municipalityId,
            interfaceType: iface,
            fullUnit: currentUnit
        });

        return iface === 'simplified';
    }, [currentUnit]);

    useEffect(() => {
        console.log('[Register] isSimplifiedMode active?', isSimplifiedMode);
    }, [isSimplifiedMode]);
    ...
    const allowedFichas = useMemo<FichaType[]>(() => {
        ...
        // Simplified Mode Override
        if (isSimplifiedMode) return ['SIMPLIFIED' as any];
    ...
    // Force Simplified Active
    useEffect(() => {
        if (isSimplifiedMode) {
            setActiveFicha('SIMPLIFIED' as any);
        }
    }, [isSimplifiedMode]);
    ...
    const isLediTarget = useMemo(() => {
        if (isSimplifiedMode) return false; // Force FALSE for simplified mode
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // LEDI/APS Validation
        if (isLediTarget && !isSimplifiedMode) {
        ...
        try {
            ...
            procedures.forEach(p => {
                ...
                if (isConsultation || activeFicha === 'INDIVIDUAL') {
            ...
            const runFai = !isSimplifiedMode && (activeFicha === 'INDIVIDUAL' || activeFicha === 'ODONTO' || activeFicha === 'DOMICILIAR');
            ...
            // ----------------------------------------------------
            if ((activeFicha === 'INDIVIDUAL' || activeFicha === ('SIMPLIFIED' as any)) && fpProcedures.length > 0) {
                ...
                savePromises.push(saveMultipleBpaRecords({
                    ...
                    originFicha: isSimplifiedMode ? 'SIMPLIFIED' : 'PROCEDIMENTOS',
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto pb-8 md:pb-0">
            <div className="mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
                ...
                {isSimplifiedMode && (
                    <div className="bg-purple-50 border-l-4 border-purple-500 p-4 rounded-r shadow-sm flex items-start gap-3">
                        <Layout className="w-5 h-5 text-purple-600 mt-0.5" />
                        <div>
                            <h3 className="text-sm font-bold text-purple-700">Modo Simplificado Ligado</h3>
                            <p className="text-xs text-purple-600 mt-1">
                                Você está registrando produção em modo simplificado. Fichas detalhadas (e-SUS/APS) e validações estritas estão desativadas para esta unidade.
                            </p>
                        </div>
                    </div>
                )}
                ...
                <Card className="p-5 border-l-4 border-l-medical-500">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        {/* NEW: Local de Atendimento Visual Selector - Hidden for ACS */}
                        {activeFicha !== 'DOMICILIAR' && !isSimplifiedMode && (
                ...
                <Card className="p-5 border-l-4 border-l-blue-500">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                        ...
                        */}
                        {isLediTarget && !isSimplifiedMode && activeFicha !== 'INDIVIDUAL' && activeFicha !== 'DOMICILIAR' && (
                            <>
                                <Select
                                    label="Turno"
