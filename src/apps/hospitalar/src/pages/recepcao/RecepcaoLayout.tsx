...
import { Hospital, LayoutDashboard, Users, LogOut, Menu, X, Bell, ChevronLeft, ChevronRight, UserPlus } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const RecepcaoLayout: React.FC = () => {
  ...
  const navItems = [
    { path: '/recepcao', icon: LayoutDashboard, label: 'Dashboard' },
    { path: '/recepcao/acolhimento', icon: UserPlus, label: 'Acolhimento' },
    { path: '/recepcao/espera', icon: Users, label: 'Fila de Espera' },
  ];
    ...
    <div className="min-h-screen bg-gray-50/50 dark:bg-gray-900 flex font-sans">
      ...
      >
        <div className={`h-16 flex items-center border-b border-gray-200 dark:border-gray-700 transition-all ${isSidebarCollapsed && !isSidebarOpenMobile ? 'justify-center px-0' : 'px-6 gap-3'}`}>
          <div className="p-2 bg-red-600 rounded-xl shadow-sm flex-shrink-0">
        ...
        <div className="flex-1 py-6 overflow-y-auto overflow-x-hidden flex flex-col gap-2 px-3">
          ...
          <nav className="space-y-1.5">
            {navItems.map((item) => {
              const isActive = location.pathname === item.path || (item.path !== '/recepcao' && location.pathname.startsWith(item.path));
              return (
                <Link
                  key={item.path}
                  to={item.path}
                ...
                >
                  <item.icon className={`w-5 h-5 flex-shrink-0 transition-colors ${isActive ? 'text-red-600 dark:text-red-400' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300'}`} />
                  {(!isSidebarCollapsed || isSidebarOpenMobile) && (
                    <span className="whitespace-nowrap">{item.label}</span>
                  )}
                  {isActive && (!isSidebarCollapsed || isSidebarOpenMobile) && (
                    <motion.div layoutId="activeNav" className="absolute right-2 w-1.5 h-1.5 rounded-full bg-red-600 dark:bg-red-400" />
                  )}
                </Link>
              );
            })}
        ...
        <div className={`p-4 border-t border-gray-200 dark:border-gray-700 transition-all ${isSidebarCollapsed && !isSidebarOpenMobile ? 'items-center flex flex-col' : ''}`}>
          {(!isSidebarCollapsed || isSidebarOpenMobile) ? (
            <div className="flex items-center gap-3 mb-4 px-2">
              <div className="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-600 dark:text-red-400 font-bold text-sm flex-shrink-0">
                {user?.name.charAt(0)}
              </div>
            ...
            <div className="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-600 dark:text-red-400 font-bold text-sm mb-4" title={user?.name}>
              {user?.name.charAt(0)}
            </div>
          )}
        ...
        <header className="h-16 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-10 sticky top-0">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden lg:block">
              <span className="text-sm font-medium text-gray-500">Módulo de Recepção</span>
            </div>
          </div>
