...
import { Bed, LayoutDashboard, Pill, Activity, LogOut, Menu, X, Bell, ChevronLeft, ChevronRight } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const EnfermariaLayout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (user.role !== 'HOSPITALAR_ENFERMARIA') {
        navigate('/login');
        return;
      }
  ...
  const navItems = [
    { path: '/enfermaria', label: 'Painel Geral', icon: LayoutDashboard },
    { path: '/enfermaria/leitos', label: 'Mapa de Leitos', icon: Bed },
  ];
      ...
      >
        <div className={`p-6 flex items-center border-b border-gray-100 dark:border-gray-700 ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
          <div className="flex items-center gap-3">
            ...
            {!isSidebarCollapsed && (
              <div>
                <h1 className="font-black text-xl tracking-tight text-gray-900 dark:text-white">Pro<span className="text-purple-600">BPA</span></h1>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Enfermaria</p>
              </div>
            )}
          </div>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const isActive = location.pathname === item.path || (item.path !== '/enfermaria' && location.pathname.startsWith(item.path));
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${isActive ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300'} transition-colors`} />
                {!isSidebarCollapsed && <span>{item.label}</span>}
                
                {isActive && !isSidebarCollapsed && (
                  <motion.div layoutId="activeNavIndicator" className="ml-auto w-1.5 h-1.5 rounded-full bg-purple-600 dark:bg-purple-400" />
                )}
              </Link>
            );
          })}
        ...
        <div className="p-4 mt-auto border-t border-gray-100 dark:border-gray-700 space-y-2">
          ...
          <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            <div className="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-900/30 border border-purple-200 dark:border-purple-800 flex items-center justify-center text-purple-700 dark:text-purple-400 font-bold flex-shrink-0">
              {userName.charAt(0)}
            </div>
            {!isSidebarCollapsed && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{userName}</p>
                <p className="text-[10px] uppercase font-bold text-purple-600 dark:text-purple-400">ENFERMARIA</p>
              </div>
            )}
          </div>
          <button 
        ...
        <header className="h-16 lg:h-20 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 flex items-center justify-between px-4 lg:px-8 z-30 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden lg:block">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Internação e Observação</h2>
              <p className="text-sm text-gray-500">Gestão Assistencial de Enfermagem</p>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 rounded-full text-sm font-medium">
...
export default EnfermariaLayout;
