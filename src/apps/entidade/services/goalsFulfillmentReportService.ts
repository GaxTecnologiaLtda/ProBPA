...
export interface GoalsFulfillmentReportOptions {
    year: string;
    startDate?: string;
    endDate?: string;
...
export const goalsFulfillmentReportService = {

    generatePdf: async (options: GoalsFulfillmentReportOptions) => {
        ...
        const drawHeader = () => {
            ...
            doc.setFont('helvetica', 'bold');
            doc.text(`Relatório de Cumprimento de Metas - Série Histórica ${options.year}`, pageWidth / 2, 20, { align: 'center' });
        ...
        const head: any[] = [
            [
                ...
                { content: 'Total', styles: { halign: 'center' as const, cellWidth: 14, fillColor: [224, 255, 255] } },
                { content: 'Meta\nPeríodo', styles: { halign: 'center' as const, cellWidth: 14, fillColor: [255, 255, 255] } },
                { content: '% Cump.', styles: { halign: 'center' as const, cellWidth: 13, fillColor: [240, 255, 240] } },
                { content: 'Falta', styles: { halign: 'center' as const, cellWidth: 14, fillColor: [255, 236, 179] } }
            ]
        ];
        ...
        options.goals.forEach(goal => {
            ...
            body.push([
                ...
                ...monthlyData.map(q => ({ content: q > 0 ? q.toString() : '0', styles: { halign: 'center' as const } })),
                { content: totalAnual.toString(), styles: { halign: 'center' as const, fontStyle: 'bold', fillColor: [230, 250, 255] } },
                { content: metaAnual.toString(), styles: { halign: 'center' as const, fontStyle: 'bold' } },
                { content: percentual, styles: { halign: 'center' as const, fontStyle: 'bold', textColor: percentColor, fillColor: percentFillColor } },
                { content: falta.toString(), styles: { halign: 'center' as const, fontStyle: 'bold', textColor: falta > 0 ? [200,0,0] : [0,100,0], fillColor: [255, 250, 230] } }
            ]);
        });
        ...
        if (options.goals.length === 0) {
            ...
            options.goals.forEach(goal => {
                ...
                for (let i = 0; i < options.reportMonths.length; i++) {
                    ...
                    Object.keys(monthStats).forEach(actualProcKey => {
                        const cleanActualProcKey = actualProcKey.replace(/\D/g, '');
                        if (cleanActualProcKey.startsWith(cleanProcKey)) {
                            monthlyTotals[i] += monthStats[actualProcKey];
                            grandTotalAnual += monthStats[actualProcKey];
                        }
                    });
                }
            });
            ...
            const overallPercentual = grandMetaAnual > 0 ? ((grandTotalAnual / grandMetaAnual) * 100).toFixed(1) + '%' : '0%';
            const overallFalta = Math.max(0, grandMetaAnual - grandTotalAnual);

            footerRow.push({ content: grandTotalAnual.toString(), styles: { halign: 'center' as const, fontStyle: 'bold', fillColor: [186, 218, 255] } } as any);
            footerRow.push({ content: grandMetaAnual.toString(), styles: { halign: 'center' as const, fontStyle: 'bold', fillColor: [186, 218, 255] } } as any);
            footerRow.push({ content: overallPercentual, styles: { halign: 'center' as const, fontStyle: 'bold', fillColor: [186, 218, 255] } } as any);
            footerRow.push({ content: overallFalta.toString(), styles: { halign: 'center' as const, fontStyle: 'bold', fillColor: [186, 218, 255] } } as any);
        ...
        doc.save(`Cumprimento_Metas_${options.municipalityName.replace(/[^a-z0-9]/gi, '_')}_${options.year}.pdf`);
    },

    generateExcel: async (options: GoalsFulfillmentReportOptions) => {
        const { utils, writeFile } = await import('xlsx');
        const sheetData: any[][] = [];
        
        sheetData.push([`Relatório de Cumprimento de Metas - Série Histórica ${options.year}`]);
        ...
        utils.book_append_sheet(wb, ws, "Cumprimento de Metas");
        
        writeFile(wb, `Cumprimento_Metas_${options.municipalityName.replace(/\s+/g, '_')}_${options.year}.xlsx`);
    }
};
