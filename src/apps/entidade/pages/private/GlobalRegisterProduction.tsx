...
import { Search, Plus, MapPin, AlertCircle, CheckCircle, FileText, UserCheck, Stethoscope, Hammer, ClipboardList, Syringe, User, Layout, Activity, ChevronUp, ChevronDown, Calendar, LayoutTemplate, WifiOff, Unlock, Loader2, History } from 'lucide-react';
...
import { collection, query, where, getDocs, limit, serverTimestamp, writeBatch, doc, getDoc, collectionGroup, orderBy } from 'firebase/firestore';
...
import { CdsColetivaForm } from '../../../producao/components/forms/CdsColetivaForm';
...
import { functions } from '../../firebase'; // Assuming functions is exported from firebase.ts, adjust if needed
import { encryptPayload } from '../../../../utils/cryptoUtils';

const saveGlobalProductionViaApi = async (dataBase: any, procedures: any[]) => {
    try {
        const payload = {
            dataBase: deepRemoveUndefined(dataBase),
            procedures: deepRemoveUndefined(procedures)
        };

        const encryptedString = await encryptPayload(payload);
        
        const saveGlobalProduction = httpsCallable(functions, 'saveGlobalProduction');
        const result = await saveGlobalProduction({ encryptedPayload: encryptedString });
        
        return (result.data as any).ids;
    } catch (error) {
        console.error("Erro na API de salvação global:", error);
        throw error;
    }
};
...
const GlobalRegisterProduction: React.FC = () => {
    const navigate = useNavigate();
    const { user, claims } = useAuth();

    // MUNICIPALITY CONFIGURATION STATE - FORCED TO SIMPLIFIED FOR MASTER PANEL
    const [interfaceType] = useState<'PEC' | 'SIMPLIFIED'>('SIMPLIFIED'); // Default to SIMPLIFIED
    const isLediTarget = false; // Never LEDI for raw BPA form

    // State for Dynamic Municipality Selection
    const [selectedMunicipality, setSelectedMunicipality] = useState<string>('');
    const [availableMunicipalities, setAvailableMunicipalities] = useState<any[]>([]);

    useEffect(() => {
        // Fetch all municipalities linked to the entity for Master/Coordenacao
        const fetchMuns = async () => {
            if (!claims?.entityId) return;
            try {
                const muns = await fetchMunicipalitiesByEntity(claims.entityId);
                const activeMuns = muns.filter((m: any) => m.active || m.status === 'active' || m.status === 'ACTIVE');
                setAvailableMunicipalities(activeMuns);
                if (activeMuns.length === 1) {
                    setSelectedMunicipality(activeMuns[0].id);
                    setHistoryFilterMunicipalityId(activeMuns[0].id);
                }
            } catch (err) {
                console.error("Error fetching master municipalities:", err);
            }
        };
        fetchMuns();
    }, [claims?.entityId]);
    ...
    const selectedUser = useMemo(() => professionals.find(p => p.id === selectedProfessionalId) || null, [professionals, selectedProfessionalId]);

    const [step, setStep] = useState<'form' | 'success'>('form');

    // Fetch Unit + Professionals + Assignments derived from selectedMunicipality
    useEffect(() => {
        const fetchContext = async () => {
            if (!claims?.entityId || !selectedMunicipality) {
                setUnits([]);
                setProfessionals([]);
                setSelectedUnitId('');
            ...
            try {
                ...
                // Filter by selectedMunicipality
                const localUnits = fetchedUnits.filter(u => u.municipalityId === selectedMunicipality);
                const localProfs = fetchedProfs.filter(p => p.assignments?.some(a => a.municipalityId === selectedMunicipality));

                setUnits(localUnits);
                setProfessionals(localProfs);

            } catch (err) {
                console.error("Error fetching context for selected municipality:", err);
            } finally {
                setLoadingSetup(false);
            }
        };
    ...
    const isVaccinationAllowed = useMemo(() => {
        if (!selectedUser?.cbo) return false;
        // 2235 (Enfermeiro), 3222 (Técnico), 225 (Médico)
        return ['2235', '3222', '225'].some(prefix => selectedUser.cbo!.startsWith(prefix));
    }, [selectedUser]);
    ...
    const [historyFilterTerm, setHistoryFilterTerm] = useState<string>('');
    const [loadingHistory, setLoadingHistory] = useState(false);
    const [historyRecords, setHistoryRecords] = useState<any[]>([]);
    const [historyFilterDate, setHistoryFilterDate] = useState<string>('');
    const [collapsedGroups, setCollapsedGroups] = useState<Record<string, boolean>>({});

    const [activeHistoryTab, setActiveHistoryTab] = useState<'ENTIDADE' | 'SUBSEDE' | 'PROFISSIONAL'>('ENTIDADE');
    ...
    const [historyFilterYear, setHistoryFilterYear] = useState<string>(currentYear.toString());
    const [historyFilterMonth, setHistoryFilterMonth] = useState<string>('all');
    // Computed value for API
    const historyFilterCompetence = useMemo(() => {
        if (historyFilterMonth === 'all') return '';
        return `${historyFilterYear}${historyFilterMonth.padStart(2, '0')}`;
    }, [historyFilterYear, historyFilterMonth]);

    const filteredHistoryRecordsBySource = useMemo(() => {
        return historyRecords.filter(r => {
            if (activeHistoryTab === 'ENTIDADE') return r.source === 'master_panel';
            if (activeHistoryTab === 'SUBSEDE') return r.source === 'subsede_panel' || r.source === 'subsede';
            if (activeHistoryTab === 'PROFISSIONAL') return ['professional_panel', 'profissional_panel', 'professional_app', 'APP', 'app', 'producao_panel'].includes(r.source);
            return false;
        });
    }, [historyRecords, activeHistoryTab]);
    const [historyFilterMunicipalityId, setHistoryFilterMunicipalityId] = useState('');
    ...
    // Update Unit info
    useEffect(() => {
        if (currentUnit && selectedUser) {
            const assignment = selectedUser.assignments?.find((a: any) => a.unitId === currentUnit.id);
            const actualCbo = assignment?.cbo || assignment?.occupation || selectedUser.cbo || selectedUser.occupation || '';
            setFormData(prev => ({
                ...prev,
                unit: currentUnit.id,
                cbo: actualCbo
    ...
    // const isLediTarget = useMemo(() => { ... }); /* Removed because it is already declared as false above */
    ...
    // Hook para carregar histórico - Modo Lazy (Exige Município selecionado)
    useEffect(() => {
        const fetchHistory = async () => {
            if (!showHistory) return;
            
            // O histórico não é carregado até que o usuário filtre um município específico, economizando rede globais da entidade.
            if (!historyFilterMunicipalityId) {
                setHistoryRecords([]);
                return;
            }
            ...
            try {
                ...
                const response = await getHistoryApi({
                    municipalityId: historyFilterMunicipalityId,
                    competenceMonth: historyFilterCompetence,
                    attendanceDate: historyFilterDate || undefined,
                    searchTerm: historyFilterTerm
                });

                const results = (response.data as any).history || [];
                setHistoryRecords(results);
            } catch (err) {
                console.error("Erro ao buscar histórico Global:", err);
            } finally {
        ...
        fetchHistory();
    }, [showHistory, historyFilterMunicipalityId, historyFilterCompetence, historyFilterDate, historyFilterTerm]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (activeFicha === 'COLETIVA') {
            ...
            try {
                ...
                // Save
                await saveGlobalProductionViaApi({
        ...
        try {
            ...
            if (runFai && (faiProcedures.length > 0 || (activeFicha === 'INDIVIDUAL' && fpProcedures.length > 0))) {
                ...
                savePromises.push(saveGlobalProductionViaApi({
                    unitId: formData.unit,
                    unitName: currentUnit?.name,
                    cbo: formData.cbo,
                    professionalId: user?.professionalId || user?.id || '',
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto pb-20">
            <div className="mb-6 flex flex-col justify-between gap-4">
                <div className="bg-amber-100 border-l-4 border-amber-500 text-amber-800 p-4 rounded-md flex items-center shadow-sm">
                    <AlertCircle className="h-6 w-6 mr-3 text-amber-600 flex-shrink-0" />
                    <span className="font-semibold text-sm">EM FASE DE CONCLUSÃO, AGUARDE...</span>
                </div>
                
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Central de Produção Manual</h1>
                        <p className="text-gray-500 dark:text-gray-400 text-sm">Lançamento simplificado e auditoria de produção global.</p>
                    </div>
                </div>

                <div className="flex border-b border-gray-200 dark:border-gray-700 w-full overflow-x-auto mt-2 scrollbar-hide">
                    <button
                        onClick={(e) => { e.preventDefault(); setShowHistory(false); }}
                        className={`flex items-center gap-2 px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${!showHistory
                            ? 'border-medical-500 text-medical-600 dark:text-medical-400 bg-medical-50/50 dark:bg-medical-900/10'
            ...
            {showHistory ? (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                    <Card className="p-4 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col md:flex-row gap-4">
                        <div className="flex-1">
                            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Município (Obrigatório)</label>
                            <div className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 flex items-center shadow-sm h-10 w-full focus-within:ring-2 focus-within:ring-medical-500">
                                <select 
                                    className="w-full text-sm font-medium text-gray-900 dark:text-white bg-transparent border-none focus:ring-0 p-0 cursor-pointer outline-none"
                                    value={historyFilterMunicipalityId} 
                                    onChange={(e: any) => setHistoryFilterMunicipalityId(e.target.value)}
                                >
                                    <option value="" className="text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800">Selecione para carregar</option>
                                    {availableMunicipalities.map((m: any) => (
                                        <option key={m.id} value={m.id} className="text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800">{m.name}</option>
                                    ))}
                                </select>
                            </div>
                        </div>
                        <div className="flex-[1.5]">
                            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Filtros de Data (Opcional)</label>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                <div className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 flex items-center shadow-sm h-10 flex-1 focus-within:ring-2 focus-within:ring-medical-500">
                                    ...
                                    <select
                                        ...
                                        className="w-full text-sm font-medium text-gray-900 dark:text-white bg-transparent border-none focus:ring-0 p-0 cursor-pointer outline-none"
                                    >
                                        <option value={currentYear.toString()} className="text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800">Ano {currentYear}</option>
                                        <option value={(currentYear - 1).toString()} className="text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800">Ano {currentYear - 1}</option>
                                        <option value={(currentYear - 2).toString()} className="text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800">Ano {currentYear - 2}</option>
                                    </select>
                                </div>
                                <div className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 flex items-center shadow-sm h-10 flex-1 focus-within:ring-2 focus-within:ring-medical-500">
                                    <select
                                        ...
                                        className="w-full text-sm font-medium text-gray-900 dark:text-white bg-transparent border-none focus:ring-0 p-0 cursor-pointer outline-none"
                                    >
                                        <option value="all" className="text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800">Todo o Ano</option>
                                        {monthNames.map((monthName, index) => (
                                            <option key={index + 1} value={String(index + 1)} className="text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800">{monthName}</option>
                                        ))}
                                    </select>
                                </div>
                                <div>
                                    <Input 
                                        ...
                                        onChange={(e: any) => setHistoryFilterDate(e.target.value)}
                        ...
                        <div className="flex-1">
                            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Busca Rápida</label>
                            <Input 
                                type="text"
                                placeholder="Paciente, CNS..." 
                                value={historyFilterTerm}
                                onChange={(e: any) => setHistoryFilterTerm(e.target.value)}
                                className="h-10 text-sm"
                            />
                        </div>
                    </Card>
                    ...
                    {loadingHistory ? (
                        <div className="flex flex-col items-center justify-center p-12 text-gray-500">
                            <Loader2 className="h-8 w-8 animate-spin text-medical-500 mb-4" />
                            <p>Buscando produções globais (pode demorar alguns segundos)...</p>
                        </div>
                    ) : historyRecords.length === 0 ? (
                        <div className="flex flex-col items-center justify-center p-12 text-gray-500 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                            <History className="h-12 w-12 text-gray-400 mb-4" />
                            {historyFilterMunicipalityId ? <p>Nenhum registro encontrado para estes filtros.</p> : <p>Selecione um município acima para carregar as produções manuais.</p>}
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {/* TABS INTERNAS DO HISTÓRICO */}
                            <div className="flex space-x-2 border-b border-gray-200 dark:border-gray-700 w-full mb-4">
                                <button
                                    onClick={() => setActiveHistoryTab('ENTIDADE')}
                                    className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 ${activeHistoryTab === 'ENTIDADE' ? 'border-purple-500 text-purple-700 dark:text-purple-400 bg-purple-50/50 dark:bg-purple-900/10' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                                >
                                    Entidade Master
                                </button>
                                <button
                                    onClick={() => setActiveHistoryTab('SUBSEDE')}
                                    className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 ${activeHistoryTab === 'SUBSEDE' ? 'border-blue-500 text-blue-700 dark:text-blue-400 bg-blue-50/50 dark:bg-blue-900/10' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                                >
                                    Subsede (Municipal)
                                </button>
                                <button
                                    onClick={() => setActiveHistoryTab('PROFISSIONAL')}
                                    className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 ${activeHistoryTab === 'PROFISSIONAL' ? 'border-emerald-500 text-emerald-700 dark:text-emerald-400 bg-emerald-50/50 dark:bg-emerald-900/10' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                                >
                                    Profissional (App)
                                </button>
                            </div>

                            {filteredHistoryRecordsBySource.length === 0 ? (
                                <div className="p-8 text-center text-sm text-gray-500 border border-dashed rounded-lg">
                                    Nenhuma produção encontrada para este filtro.
                                </div>
                            ) : (
                                (activeHistoryTab === 'PROFISSIONAL' || activeHistoryTab === 'SUBSEDE') ? (
                                    <div className="space-y-4">
                                        {Object.entries(
                                            filteredHistoryRecordsBySource.reduce((acc, record) => {
                                                const profId = record.professionalId || 'unknown';
                                                const profInfo = { name: record.professionalName || record.professionalId || 'Profissional não identificado', cbo: record.cbo || '' };
                                                if (!acc[profId]) acc[profId] = { info: profInfo, records: [] };
                                                acc[profId].records.push(record);
                                                return acc;
                                            }, {} as Record<string, { info: any, records: any[] }>)
                                        ).map(([profId, group]: [string, any]) => {
                                            const isCollapsed = !!collapsedGroups[profId];
                                            return (
                                                <div key={profId} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm">
                                                    <div 
                                                        onClick={() => setCollapsedGroups(prev => ({ ...prev, [profId]: !prev[profId] }))}
                                                        className="px-4 py-3 bg-emerald-50/50 dark:bg-emerald-900/10 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center cursor-pointer hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors select-none"
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-800 text-emerald-700 dark:text-emerald-300 flex items-center justify-center font-bold">
                                                                <User size={16} />
                                                            </div>
                                                            <div>
                                                                <h3 className="font-bold text-gray-900 dark:text-white capitalize text-sm">{group.info.name}</h3>
                                                                <p className="text-xs text-gray-500">{group.info.cbo ? `CBO: ${group.info.cbo}` : 'CBO Não Informado'}</p>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center gap-3">
                                                            <span className="text-xs font-bold bg-white dark:bg-gray-700 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 px-2 py-1 rounded-full">
                                                                {group.records.reduce((sum, r) => sum + (Number(r.quantity) || 1), 0)} procedimentos
                                                            </span>
                                                            <ChevronDown className={cn("text-gray-400 transition-transform", isCollapsed && "-rotate-90")} size={20} />
                                                        </div>
                                                    </div>
                                                    
                                                    {!isCollapsed && (
                                                        <div className="p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 bg-gray-50/30 dark:bg-gray-900/10">
                                                            {group.records.map((record, idx) => (
                                                                <Card key={record.id || idx} className="p-4 shadow-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:shadow-md transition-shadow">
                                                                    ...
                                                                    <div className="flex flex-wrap gap-2 mb-3">
                                                                        <div className="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[10px] font-mono px-2 py-0.5 rounded">
                                                                    ...
                                                                    <div className="mt-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                                                                        <div className="flex items-center gap-2 mb-1">
                                                                            <span className="inline-flex items-center px-1.5 py-0.5 rounded-sm text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 font-mono">
                                                                                {record.procedureCode}
                                                                            </span>
                                                                            <span className="text-xs font-medium text-gray-700 dark:text-gray-300 line-clamp-1">{record.procedureName}</span>
                                                                        </div>
                                                                        <div className="flex justify-between items-end mt-3">
                                                                            <div className="text-[10px] text-gray-500 flex flex-col">
                                                                                <span><strong>Data:</strong> {record.attendanceDate ? record.attendanceDate.split('-').reverse().join('/') : record.date?.split('-').reverse().join('/')}</span>
                                                                                <span className="text-emerald-600 dark:text-emerald-400 font-medium">{record.unitName || record.unitId}</span>
                                                                            </div>
                                                                            <div className="text-[10px] font-bold text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                                                                                Qtd: {record.quantity}
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                        {filteredHistoryRecordsBySource.map((record, idx) => (
                                            <Card key={record.id || idx} className="p-4 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
                                                <div className="flex justify-between items-start mb-2">
                                                    <h4 className="font-bold text-gray-900 dark:text-white text-sm line-clamp-1">
                                                        {record.patientName || 'Não Informado'}
                                                    </h4>
                                                ...
                                                <div className="mt-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className={cn(
                                                            "inline-flex items-center px-1.5 py-0.5 rounded-sm text-[10px] font-bold border font-mono",
                                                            record.source === 'master_panel' ? "bg-purple-100 text-purple-800 border-purple-200" :
                                                    ...
                                                    <div className="flex justify-between items-end mt-3">
                                                        <div className="text-[10px] text-gray-500 flex flex-col">
                                                            <span><strong>Data:</strong> {record.attendanceDate ? record.attendanceDate.split('-').reverse().join('/') : record.date?.split('-').reverse().join('/')}</span>
                                                            <span className="text-gray-600 dark:text-gray-400 font-medium line-clamp-1" title={record.professionalName}>{record.professionalName?.split(' ')[0] || `Prof. ${record.professionalId?.slice(0,4)}`}</span>
                                                        </div>
                                                        <div className="text-[10px] font-bold text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                                                            Qtd: {record.quantity}
                                                        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
                ...
                <Card className="p-5 border-l-4 border-l-medical-500">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        {/* NEW: Component for selecting Global Municipality */}
                        <div className="sm:col-span-2">
                            <Select
                                label="Município de Lotação"
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            >
                                ...
                                {professionals
                                    ...
                                    .map(p => {
                                        // Try to get CBO specific to the selected unit first, then fallback to primary CBO/Occupation
                                        const assignment = p.assignments?.find(a => a.unitId === selectedUnitId);
                                        const activeCbo = assignment?.cbo || assignment?.occupation || p.cbo || p.occupation;
                                        const cboDisplay = activeCbo ? activeCbo : 'Sem CBO configurado';
                ...
                {activeFicha !== 'DOMICILIAR' && activeFicha !== 'COLETIVA' && activeFicha !== 'VACINACAO' && activeFicha !== 'INDIVIDUAL' && (
                    <div className="space-y-4">
                        ...
                        <div className="space-y-4">
                            ...
                            {/* Add Button */}
                            <Button variant="outline" onClick={handleAddProcedure} className="w-full border-dashed border-2 py-6 border-orange-200 text-orange-600 hover:bg-orange-50 hover:border-orange-300 dark:border-orange-800 dark:text-orange-400 dark:hover:bg-orange-900/20">
...
export default GlobalRegisterProduction;