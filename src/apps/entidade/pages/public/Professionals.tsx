...
import { Plus, Search, User, Stethoscope, MapPin, Edit2, Trash2, Filter, Building2, ChevronDown, Briefcase, RefreshCw, Key, Mail, CheckCircle, Send, X } from 'lucide-react';
...
const Professionals: React.FC = () => {
  ...
  const toggleStatus = async (prof: Professional) => {
    // For simplicity, open edit modal to manage status per assignment
  ...
  const handleGrantAccess = async (resetPassword = false) => {
    ...
    try {
      ...
      const result = await setClaims({
        ...
        email: selectedProfForAccess.email,
        entityId: claims.entityId,
        entityName: entity?.name,
  ...
  const handleAddAssignment = () => {
    ...
    // Find municipality name. If not in list (Public Entity case), try to use unit's data or fallback
    const municipality = municipalities.find(m => m.id === unit.municipalityId);
    const municipalityName = municipality?.name || 'Município Vinculado';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Corpo Clínico (Público)</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Gestão de profissionais organizada por Unidade de Saúde.
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      >
        <div className="space-y-6 text-center">
          ...
          <div>
            ...
            {emailSent && generatedPassword && (
              <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                ...
                <p className="text-xs text-green-600 dark:text-green-400 mt-2">
                  Esta senha é temporária e não será exibida novamente.
                </p>
              </div>
            )}
            ...
            {!emailSent && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 px-6">
                O profissional receberá as credenciais de acesso ao Painel de Produção no e-mail cadastrado.
              </p>
            )}
          </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700/30 p-4 rounded-lg border border-gray-100 dark:border-gray-700">
            ...
            <div className="space-y-3 mb-4">
              {formAssignments.map((assignment, index) => (
                <div key={index} className="flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm">
                  <div>
            ...
            <div className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-dashed border-gray-300 dark:border-gray-600">
              <p className="text-xs font-bold text-gray-500 uppercase mb-2">Adicionar Novo Vínculo</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="md:col-span-2 flex justify-end">
                  ...
                  >
                    <Plus className="w-3 h-3 mr-1" /> Adicionar Vínculo
                  </Button>
                </div>
              </div>
            </div>
